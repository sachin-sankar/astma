...
Mathematical definitions and statistical formulations used in the analytical pipeline.

## 1. High-earner classification

The high-earner threshold $\tau_{90}$ is defined as the empirical 90th percentile of total lifetime platform earnings across all valid profiles:
...
## 2. Skill combinations and association mining
...
- Co-occurrence Lift:
  ...
  $\text{Lift} > 1$ indicates that the skills co-occur more frequently than expected under independence.

### 2.2 High-earner proportion and lift
...
To avoid division by zero in sparse cells, add continuity correction $\delta = 0.5$ if $\min(a, b, c, d) = 0$:
...
Reject $H_0$ if $p_{(i)}^{\text{adj}} < 0.05$.

## 4. Controlled regressions

To evaluate whether bundle premiums persist when controlling for profile characteristics:
...
## 6. Text analysis and topic modeling
...
Generative Dirichlet-multinomial mixture model decomposing document-term matrix $X$ into document topic distributions $\theta_d$ and topic word distributions $\phi_k$.
